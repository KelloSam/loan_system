defmodule MiwayCreditCore.Loans.CollateralTest do
  ...
  describe "list_collaterals_for_account/1 and total_collateral_value/1" do
    test "lists only the given account's collateral and sums the value" do
      ...
      {:ok, _} = Loans.create_collateral(%{"loan_account_id" => account_id, "type" => "vehicle", "description" => "Car", "estimated_value" => "20000.00"})
      {:ok, _} = Loans.create_collateral(%{"loan_account_id" => account_id, "type" => "land", "description" => "Plot", "estimated_value" => "30000.00"})
      {:ok, _} = Loans.create_collateral(%{"loan_account_id" => other_application.loan_account.id, "type" => "electronics", "description" => "Laptop", "estimated_value" => "5000.00"})

      results = Loans.list_collaterals_for_account(account_id)
      assert length(results) == 2

      assert Decimal.equal?(Loans.total_collateral_value(account_id), Decimal.new("50000.00"))
    end

    test "total_collateral_value/1 is zero when no collateral is recorded" do
      application = approved_application_fixture()
      assert Decimal.equal?(Loans.total_collateral_value(application.loan_account.id), Decimal.new("0.00"))
    end
  end
  ...
  describe "get_collateral!/1 and delete_collateral/1" do
    test "fetches and deletes a collateral record" do
      application = approved_application_fixture()
      {:ok, collateral} = Loans.create_collateral(%{"loan_account_id" => application.loan_account.id, "type" => "other", "description" => "Jewelry", "estimated_value" => "1500.00"})
