defmodule MiwayCreditCore.Risk.AffordabilityCalculatorTest do
  ...
  describe "assess_affordability/3" do
    test "passes when the proposed installment stays within the ratio" do
      ...
      assert :ok =
               Risk.assess_affordability(scope, application_stub(customer, "1000", 12), product_stub("18.00", "40.00"))
    end
    ...
    test "fails when the proposed installment alone exceeds the ratio" do
      ...
      assert {:error, :affordability_exceeded} =
               Risk.assess_affordability(scope, application_stub(customer, "10000", 12), product_stub("18.00", "40.00"))
    end
    ...
    test "passes right at the threshold, fails just over it" do
      ...
      assert :ok =
               Risk.assess_affordability(scope, application_stub(customer, "4000", 12), product_stub("0", "40.00"))

      assert {:error, :affordability_exceeded} =
               Risk.assess_affordability(scope, application_stub(customer, "5000", 12), product_stub("0", "40.00"))
    end
    ...
    test "fails closed when the business customer has no annual_turnover on file" do
      customer = customer_fixture(%{"customer_type" => "business", "id_type" => "company_registration"})
      scope = %Scope{organisation_id: customer.organisation_id}
    ...
    test "counts an existing active account's monthly obligation toward the ratio" do
      ...
      assert {:error, :affordability_exceeded} =
               Risk.assess_affordability(scope, application_stub(customer, "3000", 12), product_stub("18.00", "40.00"))
    end
  end

  defp application_stub(customer, amount, term_months) do
    %LoanApplication{customer_id: customer.id, requested_amount: Decimal.new(amount), requested_term_months: term_months}
  end

  defp product_stub(interest_rate, max_debt_to_income_percent) do
    %LoanProduct{interest_rate: Decimal.new(interest_rate), max_debt_to_income_percent: Decimal.new(max_debt_to_income_percent)}
  end
end
