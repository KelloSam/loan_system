defmodule MiwayCreditCore.CrossOrganisationIsolationTest do
  ...
  defp build_upload(content) do
    path =
      Path.join(System.tmp_dir!(), "isolation_test_upload_#{System.unique_integer([:positive])}")
  ...
  defp build_world do
    ...
    {:ok, transaction} =
      Payments.record_payment(scope, valid_payment_attrs(account, %{"amount" => "50.00"}))
    ...
    {:ok, kyc_document} =
      Customers.create_kyc_document(customer, build_upload("secret bytes"), "nrc_copy", admin.id)
  ...
  describe "context layer — fetch-by-id across organisations" do
    ...
    test "Customers.get_customer!/2 refuses another organisation's customer", %{
      org_a: a,
      org_b: b
    } do
    ...
    test "Applications.get_application!/2 refuses another organisation's application", %{
      org_a: a,
      org_b: b
    } do
      assert_raise Ecto.NoResultsError, fn ->
        Applications.get_application!(a.scope, b.application.id)
      end
    end

    test "Applications.get_collateral!/2 refuses another organisation's collateral", %{
      org_a: a,
      org_b: b
    } do
      assert_raise Ecto.NoResultsError, fn ->
        Applications.get_collateral!(a.scope, b.collateral.id)
      end
    end
    ...
    test "Lending.get_installment!/2 refuses another organisation's installment", %{
      org_a: a,
      org_b: b
    } do
      assert_raise Ecto.NoResultsError, fn ->
        Lending.get_installment!(a.scope, b.installment.id)
      end
    end

    test "Payments.get_transaction!/2 refuses another organisation's payment transaction", %{
      org_a: a,
      org_b: b
    } do
      assert_raise Ecto.NoResultsError, fn ->
        Payments.get_transaction!(a.scope, b.transaction.id)
      end
    end

    test "Customers.get_kyc_document!/2 refuses another organisation's KYC document", %{
      org_a: a,
      org_b: b
    } do
      assert_raise Ecto.NoResultsError, fn ->
        Customers.get_kyc_document!(a.scope, b.kyc_document.id)
      end
    end

    test "list/aggregate functions never include another organisation's rows", %{
      org_a: a,
      org_b: b
    } do
      ...
      refute b.collateral.id in Enum.map(
               Applications.list_collaterals_for_account(a.scope, b.account.id),
               & &1.id
             )
    ...
    test "a platform administrator's :all scope legitimately sees every organisation", %{
      org_a: a,
      org_b: b
    } do
      ...
      assert Applications.get_application!(platform_scope, a.application.id).id ==
               a.application.id

      assert Applications.get_application!(platform_scope, b.application.id).id ==
               b.application.id
    end
  end
  ...
  describe "HTTP layer — a real staff session cannot reach another organisation's records by URL" do
    ...
    test "the same staff session correctly reaches its own organisation's records", %{
      conn: conn,
      org_a: a
    } do
    ...
    test "GET a KYC document download for another organisation's customer is refused, not served",
      ...
      assert_error_sent 404, fn ->
        get(
          conn,
          ~p"/admin/customers/#{b.customer.id}/kyc_documents/#{b.kyc_document.id}/download"
        )
      end
    end

    test "the same staff session can download its own organisation's KYC document", %{
      conn: conn,
      org_a: a
    } do
      conn =
        conn
        |> login(a.staff_user)
        |> get(~p"/admin/customers/#{a.customer.id}/kyc_documents/#{a.kyc_document.id}/download")
