defmodule MiwayCreditCoreWeb.AuditLogHTML do
  ...
  def event_color(event) do
    cond do
      String.ends_with?(event, ["_approved", "_verified", "_success"]) ->
        "text-green-700 bg-green-50"

      String.ends_with?(event, ["_rejected", "_failure", "_failed", "_removed", "_deleted"]) ->
        "text-red-600 bg-red-50"

      String.starts_with?(event, "payment_") ->
        "text-gold-700 bg-gold-50"

      String.starts_with?(event, ["loan_application_", "loan_account_"]) ->
